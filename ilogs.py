...
filename_parser =re.compile(r"^(\d{8})?_?(\d{2})?_?(\D+)(\d{4}.?\d{2}.?\d{2})?(\d{2}.?\d{2}.?\d{2})?.?([a-zA-Z]*).?(\d*).?([a-zA-Z]*)")
...
for i in range(1,2):
    ...
    for root, dirs, files in os.walk(server_name):
        ...
        for item in files:
            try:
                name = filename_parser.search(item).group(3)
            except:
                print item
...
renderer1.set_property( 'editable', False )
