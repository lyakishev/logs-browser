from evs import *
import threading
import pygtk
pygtk.require("2.0")
import gtk, gobject

class LogWorker(threading.Thread):

    def __init__(self, fltr, inq, outq):
        threading.Thread.__init__(self)
        self.ret_self = lambda l: l
        self.fltr = fltr
        self.inqueue = inq
        self.outqueue = outq
        self.type_func = self.fltr['types'] and self.f_type or self.ret_self
        self.date_func = fltr['date'] and self.f_date or self.ret_self
        self.content_like_func = fltr['content'][0] and self.f_likecont or self.ret_self
        self.content_notlike_func = fltr['content'][1] and self.f_notlikecont or self.ret_self

    def last_gen(self, gen):
        if self.fltr['last'] != 0:
            counter = 0
            for l in gen:
                if l:
                    if counter < self.fltr['last']:
                        counter+=1
                        yield l
                    else:
                        return
        else:
            for l in gen:
                if l:
                    yield l
    def f_date(self, l):
        if l:
            if l['the_time']<=self.fltr['date'][1] and l['the_time']>=self.fltr['date'][0]:
                return l

    def date_generator(self, func, gen):
        try:
            for l in gen:
                if l['the_time']<self.fltr['date'][0]:
                    return
                else:
                    yield func(l)
        except IndexError:
            for l in gen:
                yield func(l)

    def f_likecont(self, l):
        if l:
            if eval(self.fltr['content'][0]):# in l['msg']:
                return l

    def f_notlikecont(self, l):
        if l:
            if self.fltr['content'][1] not in l['msg']:
                return l

    def f_type(self, l):
        if l:
            if l['evt_type'] in self.fltr['types']:
                return l

    def generators(self, comp, log):
        self.f1 = (self.date_generator(self.date_func, getEventLogs(comp, log)))
        self.f2 = (self.type_func(l) for l in self.f1)
        self.f3 = (self.content_notlike_func(l) for l in (self.content_like_func(l1) for l1 in self.f2))
        self.f4 = self.last_gen(self.f3)
        return self.f4

    def run(self):
        #print "%s %s acquire" % (self.log, self.comp)
        while 1:
            if self.inqueue.empty():
                break
            comp, log = self.inqueue.get_nowait()
            for l in self.generators(comp, log):
                if l:
                    self.outqueue.put_nowait((l['the_time'], l['computer'], l['logtype'], l['evt_type'], l['source'], l['msg']))

class ModFiller(threading.Thread):
    def __init__(self, model, q, stq):
        self.model = model
        self.q = q
        self.stq = stq
        threading.Thread.__init__(self)

    def run(self):
        while 1:
            data = self.q.get_nowait()
            gtk.gdk.threads_enter()
            self.model.append(data)
            gtk.gdk.threads_leave()
    
